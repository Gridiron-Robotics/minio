#!/usr/bin/env bash
#
# lib.sh - shared helpers for the Gridiron minio-overlay hardening gates.
#
# Adapted from the estate's lago/repo-gates/lib.sh so output and exit codes match
# the other estate gates. It gives us:
...
#
# Exit-code convention used by the gate:
#   0 = everything passed
#   1 = at least one real failure (broken code / config)
#   2 = nothing failed, but at least one check was skipped (tool/env missing)
#
# STRICT=1 turns every SKIP into a FAIL. Use it in CI and when you want the real
# "is this actually production ready?" verdict. The whole point is to NOT trust a
# green that was only green because checks were skipped.
#
# NOTE: this file is SOURCED, so it deliberately does NOT run `set -e`/`set -u`.
# A sourced library must not change shell flags for its caller.
